...
export default async function AnalystCard({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    ...
    <main className="max-w-2xl mx-auto p-8">
      {error && (
        <p className="text-red-600 mb-4">Veri yüklenirken bir hata oluştu.</p>
      )}
      ...
      <p className="text-gray-600 mb-6">Tahmin Sayısı: {analyst?.sample_size ?? "-"}</p>

      <h2 className="text-xl font-semibold mb-3">Tahminler</h2>
     {predictions && predictions.length > 0 ? (
        ...
        <p className="text-gray-500 mb-8">Bu analistin henüz tahmini yok.</p>
      )}

      <h2 className="text-xl font-semibold mb-3">Performans Grafiği</h2>
